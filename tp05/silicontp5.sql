...
-- select c.id, c.nombre,c.apellido,p.fecha,p.cantidad from clientes as c; 
-- (select clientes.id, max(cantidad) as cantidades, fecha from pedidos group by id, fecha);
